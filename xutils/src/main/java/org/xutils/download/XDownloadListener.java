package org.xutils.download;

import android.view.View;

import org.xutils.common.Callback;
import org.xutils.x;

import java.io.File;

/**
 * Created by wyouflf on 15/11/10.
 */
public abstract class XDownloadListener {

    protected XDownloadInfo mXDownloadInfo;
    public XDownloadListener(XDownloadInfo downloadInfo) {
        this(null, downloadInfo);
    }
    public XDownloadListener(View view, XDownloadInfo downloadInfo) {
        this.mXDownloadInfo = downloadInfo;
        x.view().inject(this, view);
    }

    public final XDownloadInfo getXDownloadInfo() {
        return mXDownloadInfo;
    }

    public void update(XDownloadInfo downloadInfo) {
        this.mXDownloadInfo = downloadInfo;
    }

    public abstract void onWaiting();

    public abstract void onStarted();

    public abstract void onLoading(long total, long current);

    public abstract void onSuccess(File result);

    public abstract void onError(Throwable ex, boolean isOnCallback);

    public abstract void onCancelled(Callback.CancelledException cex);
}
